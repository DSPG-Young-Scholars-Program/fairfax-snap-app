...
data_long <- weights_geo_inc %>% filter(income_cat == "$50,000 to $59,999") %>% pivot_longer(c(monthly_mean_COVID_benefit, monthly_mean_SNAP_benefit), names_to = "Benefit_Type", values_to = "Dollars_Received")
#View(data_long)

mean_benefit_df <- data_long  %>% group_by(Benefit_Type) %>% summarize(Mean=mean(Dollars_Received, na.rm = TRUE))
  

g <- ggplot(data=data_long, aes(x=Dollars_Received, group=Benefit_Type, fill=Benefit_Type)) + geom_density(adjust=1.5, alpha=.5) + scale_fill_viridis(option = "D", discrete=TRUE, end=.6) + geom_vline(data=mean_benefit_df, aes(xintercept=Mean, color="#232D4B"), linetype="dashed", linewidth=1) + theme_bw() + labs(title="Monthly Benefit for Yearly Income $50,000 to $59,999", x="Monthly Average Benefit / Household (Dollars)")
ggplotly(g)
```
...
```{r, echo=FALSE}
dat = weights_geo_inc %>% filter(income_cat == "$50,000 to $59,999")
