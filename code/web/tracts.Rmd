...
## Average monthly benefit of SNAP-Covid in Fairfax County (2020)
...
#palette colors
pal <- colorBin("viridis", weights_geo$monthly_mean_COVID_benefit)
...
leaflet(weights_geo , options = leafletOptions(attributionControl = FALSE)) %>%
  ...
  addControl("Average monthly benefit of SNAP-Covid", "topright") %>%
  addLegend("bottomright", pal, weights_geo$monthly_mean_COVID_benefit, opacity = .7) %>%
  addPolygons(
    fillColor = pal(weights_geo$monthly_mean_COVID_benefit), fillOpacity = .7, weight = 1, color = "#000",
    ...
    label = paste0(
      ...
      "Average monthly benefit of new policy SNAP: $", round(weights_geo$monthly_mean_SNAP_benefit,2), "<br/>",
...
## SNAP Eligibility and average SNAP benefit per month
