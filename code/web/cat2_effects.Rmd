...
data_long <- weights_geo_inc %>% filter(income_cat == "$35,000 to $49,999") %>% 
...
gg <- ggplot(data = data_long  ) +  
 geom_density(aes(x=Dollars_Received, color=Benefit_Type, fill=Benefit_Type, 
  ...
  theme_bw() + 
  labs(title="Monthly SNAP Benefit for Households with Income $35,000 to $49,999", x="Monthly Average Benefit (Dollars)") 
...
```{r, echo=FALSE}
dat = weights_geo_inc %>% filter(income_cat == "$35,000 to $49,999") %>% mutate(people_represented = round(households_represented*average_hh_size, 2))
...
```{r, echo=FALSE}
at_risk_hh = dat %>% filter(households_represented > 5) %>% mutate(`Monthly Benefit Loss` = `Monthly mean COVID benefit` - `Monthly mean SNAP benefit`) %>% arrange(desc(`Monthly Benefit Loss`)) %>% rename(`Average Household Size` = average_hh_size) %>% rename(`Households Represented` = households_represented) %>% rename(`Household Eligibility Proportion` = SNAP_eligibility_proportion) %>% select(NAME, `Household Eligibility Proportion`, `Monthly Benefit Loss`, `Households Represented`, `Average Household Size`) %>% slice(1:5) %>% gt() %>% tab_header(
    title = md("Tracts with Greatest Monthly Benefit Losses per Household"),
    subtitle = md("at income level **$35,000 to $49,999**")
  )
at_risk_hh
```

```{r, echo=FALSE}
hp <- c("Census Tract 4405.03, Fairfax County, Virginia", "Census Tract 4607.01, Fairfax County, Virginia", "Census Tract 4619.02, Fairfax County, Virginia", "Census Tract 4522, Fairfax County, Virginia")
