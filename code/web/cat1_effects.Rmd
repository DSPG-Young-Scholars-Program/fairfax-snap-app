...
```{r, echo=FALSE}
at_risk_hh = dat %>% filter(households_represented > 5) %>% mutate(`Monthly Benefit Loss` = `Monthly mean COVID benefit` - `Monthly mean SNAP benefit`) %>% mutate(`Proportion Income Lost per Household` = 1 - (monthly_mean_income - `Monthly Benefit Loss`)/monthly_mean_income) %>% arrange(desc(`Monthly Benefit Loss`)) %>% rename(`Average Household Size` = average_hh_size) %>% rename(`Households Represented` = households_represented) %>% select(NAME, `Monthly Benefit Loss`, `Households Represented`, `Average Household Size`, `Proportion Income Lost per Household`) %>% slice(1:5) %>% gt() %>% tab_header(
